...
class VideoGeneratorRepositoryImpl @Inject constructor(
    ...
    override suspend fun generateConcatenatedVideo(
        ...
        val cmd = buildList {
            ...
            addAll(listOf("-c:v", "mpeg4", "-q:v", "3", "-an"))
            addAll(listOf("-r", "30"))
            add(finalOutputPath)
        }
    ...
    private fun createVideoSlices(
        ...
        videoFiles.forEachIndexed { i, videoPath ->
            ...
            onProgress(
                15 + (i.toFloat() / videoFiles.size * 40).toInt(),
    ...
    override suspend fun generateImageLoopVideo(
        ...
        val textLines: List<String>? = config.textCategory?.let { getRandomText(it).text.split("\n") }
        ...
        repeat(slicesNeeded) { index ->
            ...
            val cmd = buildList {
                ...
                addAll(listOf("-map", "[out]", "-c:v", "mpeg4", "-q:v", "3", "-pix_fmt", "yuv420p", tempVideoPath))
            }
    ...
    private fun executeFFmpeg(args: List<String>) {
        ...
        if (!ReturnCode.isSuccess(session.returnCode)) {
            error("FFmpeg failed: ${session.allLogsAsString}")
        }
    }

    private fun probeVideo(file: File): Triple<Double, Int, Int> {
        val session = FFprobeKit.getMediaInformation(file.absolutePath)
        val info = session.mediaInformation ?: return Triple(0.0, 0, 0)
        val video = info.streams?.firstOrNull { it.type == "video" } ?: return Triple(0.0, 0, 0)
